...
## D32 派发权移交用户：主 Agent 不再自行派发子 Agent

> 触发：用户指令（2026-09-13）——「以后暂时不用子 Agent，我来派」。

### D32.1 [裁决]

- **主 Agent 不再自行派发子 Agent**：`headless`（`dsh --profile headless`）、`subagent`、`workflow` 三条通道**一律不再由主 Agent 主动发起**。
- **派发由用户执行**（用户在**新会话**里派，见 `docs/spec/34-module-dispatch.md` §一）。

### D32.2 主 Agent 保留的职责（不变）

1. **写与维护任务书**：`.scratch/main/mimo-<模块>-task.md`，按 `34-module-dispatch.md` 的**六段模板**；
2. **亲跑门复核**：子 Agent 落地后由主 Agent 执行该模块的门，**只看原始输出与退出码**（D14），不采信自述；
3. **选择性提交**（D15：Agent 禁 git，提交由主 Agent 做），并把**未验证项如实写进台账**；
4. **更新台账** `docs/reports/module-agents.md`（模块 → 负责 Agent → 任务书 → 状态 → 门的结果）。

### D32.3 与旧口径的关系

- **D12'**（子 Agent 默认 Mimo、headless 机制）**保留为备用能力**，但默认不由主 Agent 使用；模型路由结论（D27.1 修好的真 Mimo）仍然有效。
- **D28.2** 的补偿义务**部分保留**：由主 Agent 在**回收**时给出会话 id、模型、产物路径与门的证据；「派发」环节不再由主 Agent 执行。
- 用户仍可随时要求主 Agent 恢复派发（一句话即可）。

---

---

## D33 约定即数据、数据即门：把"边界"从散文变成断言

> 触发：用户批评（2026-09-13）——「这就是不完全模块化的后果，子 Agent 可以随意改约定」。
> 结论：**不是模块化不彻底，而是边界只写在文档里**。

### D33.1 [实测] 六条边界，原先只有一条是机器强制的

| 边界 | 原先靠什么 | 能否被随意改 |
|---|---|---|
| 契约三处一致（spec/TS/Rust） | ✅ `check-contract.mjs` **强制** | ❌ 从未被绕过 |
| workspace 成员（谁在门里） | 文档 + 主 Agent 事后复核 | ✅ **M11 把 `crates/princess-plugins` 从根成员删掉**，该模块静默掉出 CI（= BUG-009 那种「没有门的模块」） |
| 依赖（改 `[workspace.dependencies]`） | 任务书禁令 | ✅ M11 加了 `tempfile`（本意是躲开共享 `Cargo.lock`，属"好心违规"） |
| 目录所有权 | 任务书 + 复核 | ✅ 无任何检查 |
| 负样本手法（禁 `mv` 真文件） | 任务书禁令（D28.3） | ✅ 无任何检查（P-F1 已违规一次，`mv .toolchain/bin/jdtls → .hidden`） |
| 模块表本身 | 主 Agent 手写 | ✅ 谁都能改，且无人比对 |

**唯一守住的那条恰好证明模型可行**：把边界写成断言，成本只有几十行。

### D33.2 [裁决] 落地：`docs/spec/ownership.toml`（数据）+ `scripts/check-boundaries.py`（门）

- **数据**：`docs/spec/ownership.toml` = 模块 → 允许 glob（`allow`）、共享资源（`shared`）、
  故意独立的 workspace（`independent_workspaces` + **必须登记"由哪个门覆盖"**）、治理文件（`governance`）。
- **门**：`scripts/check-boundaries.py` 做三类检查，违规即非零：
  1. **workspace 成员**：`crates/*` 必须在根成员里；`apps/*` 要么是根成员，要么登记为独立 workspace
     **且必须被某个门覆盖**（"掉出门外"与"有意独立"从此可区分）；
  2. **工具链禁令**：变动集与文件系统里不得出现 `.toolchain/**` 的改名/删除/`.hidden` 残留（D28.3 事故特征）；
  3. **模块所有权**：`--agent M11` 时变动集必须全部落在该模块 `allow` 内；`shared` 需显式授权；`governance` 任何 Agent 都不许碰。
- **接入**：`scripts/ci-gate.sh` 第 7 步，**永远运行**（不编译任何东西，最便宜也最关键）。

### D33.3 [裁决] 治理文件受保护（不动点）

`ownership.toml`、`check-boundaries.py`、`ci-gate.sh`、`00-decisions.md`、`34-module-dispatch.md`
**只允许主 Agent 修改**。否则就是"被治理者改治理规则"，门再厚也没用。

### D33.4 [裁决] 任务书的所有权段改为**生成**

`python3 scripts/check-boundaries.py --print-ownership M11` 直接产出可贴进任务书的所有权段 ——
口径不再手写，从源头消除"文档与数据漂移"。

### D33.5 [实测] 所有权检查需要**派发基线**（使用条件，必须写进口径）

三路同时改树时，整棵树的 `git status` 混着多人的改动，所有权检查必然误报。
因此：**派发前** `--record-baseline <模块>` 记一次基线，**复核时** `--agent <模块> --baseline …` 把
"派发前就已脏"的路径扣除；不记基线时只跑第 1、2 类（它们对整棵树永远有效）。

### D33.6 诚实的覆盖缺口

- `apps/native`（M12 的独立 workspace）**尚未接入 `ci-gate.sh`** —— 已在 `ownership.toml` 的
  `independent_workspaces.coverage` 里**显式登记为已知缺口**（而不是靠静默漏掉）。P-E1 落地后由主 Agent 补进门。
- 门能管**行为**（改了哪些文件、加了什么依赖），管不了**意图**；它会把"为好意而越界"也拦下来 —— 这是对的，
  因为那种好意的代价正是"模块掉出门外"。

### D33 通用教训

**文档是散文，散文不设防。** 边界要能守得住，必须①落成数据、②有断言、③由不受被治理者控制的一方持有。
这三条都做到之后，"子 Agent 随意改约定"这件事在**它落地之前**就会红。
