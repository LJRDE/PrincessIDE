...
| **M13** | **语言模块层**（新增） | 把「语言支持」做成可插拔：manifest + 适配器（LSP / 构建 / 运行 / 调试） | 新 crate `crates/princess-lang/` + `languages/*.toml` | 新增：manifest 校验 + 「缝能容纳既有 C 路径」回归 + 每门语言自带门 | **需求已冻结（D31）；待做（P-F）**；Java 为第一个实例，见 `33-language-modules.md` |
...
9. **语言是模块，不是分叉**：内核路径（C+asm）保持主路径与参考实现；任何语言模块不得改变内核路径行为与断言常量（D3/D26）。
