...
| **M8** | **前端外壳 / 视图** | 布局 + IPC 接线 + **视图注册表**（每个视图自注册） | `apps/desktop/src/**` | `pnpm -C apps/desktop exec vitest run` + `tsc --noEmit` + `pnpm build` | 已有；**注册表 = P-A 待做** |
| **M9** | **桌面壳** | Tauri 窗口、24 命令路由、事件桥、capabilities | `apps/desktop/src-tauri/**` | `cargo build` + `cargo test`（在 src-tauri 内，独立工作区） | 已有；**其测试不在任何门里** |
...
| 模块 | 现在有门吗 | 缺什么 |
|---|---|---|
| M0 / M2 / M3 / M4 / M5 / M6 | ✅ `cargo test` | **契约校验不在 CI 里**（只手工跑） |
| M1 | ⚠️ 手工 | 无 CI 调用（GUI 段与版本放宽是本轮才补） |
| M7 | ⚠️ 手工 | `smoke-ci.sh` 不在任何 CI |
| M8 | ❌ | 前端 **102 个测试不在任何门里** |
| M9 | ❌ | 外壳测试**不在任何门里**（workspace 独立，`cargo test --workspace` 覆盖不到） |
| M10 / M11 / M12 | — | 尚不存在 |

**一句话结论：仓库里没有任何 CI 配置**。先把 M8 / M9 / M0 拉进一道门，再谈模块迁移（D29.4：门先于切割）。
