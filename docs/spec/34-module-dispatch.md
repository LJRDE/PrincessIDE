...
2. **所有权白名单 / 黑名单**：**不要手写** —— 用生成器产出（D33.4）：
   `python3 scripts/check-boundaries.py --print-ownership <模块>`，它读的是权威数据
   `docs/spec/ownership.toml`。跨模块改动需主 Agent 显式授权（即在 `allow` 里列出）。
...
| **边界门必须通过**：`ci-gate.sh` 第 7 步（`check-boundaries.py`）永远运行 | **D33** | 它把"成员关系 / 工具链禁令 / 模块所有权"变成断言；此前这三条只是文档 |
| **治理文件只允许主 Agent 改**：`ownership.toml` / `check-boundaries.py` / `ci-gate.sh` / `00-decisions.md` / `34-module-dispatch.md` | D33.3 | 否则门会被被治理者拆掉 |
| **派发前记基线**：`check-boundaries.py --record-baseline <模块>`；复核时 `--agent <模块> --baseline …` | D33.5 | 多路并行时整棵树的 diff 混着多人改动，不扣基线必然误报 |
