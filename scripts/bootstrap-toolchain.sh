...
#
# grub-pc-bin is the BIOS platform for GRUB (i386-pc modules).  Without it
# grub-mkrescue sees only the host's x86_64-efi tree and silently emits a
# UEFI-only ISO; QEMU's "-cdrom ... -boot d" boots BIOS, so the guest never
# started and the sole symptom was an empty serial log.  See
# install_grub_mkrescue_launcher() for the rest of that fix.
PKGS="qemu-system-x86 nasm clang clangd lld gdb xorriso mtools grub-pc-bin"
#
...
# =============================================================================
# 2c. grub-mkrescue: make the BIOS platform modules visible
# =============================================================================
# grub-mkrescue builds an ISO for every platform directory it finds under
# /usr/lib/grub.  The BIOS platform (i386-pc) ships in grub-pc-bin; when only
# grub-efi-amd64-bin is installed, /usr/lib/grub holds x86_64-efi alone and
# grub-mkrescue silently emits a **UEFI-only** ISO.  QEMU's
# "-cdrom ... -boot d" boots BIOS, so the guest never starts and the only
# symptom is an empty serial log -- which reads like a kernel or ISO bug rather
# than a missing package.  The El Torito record is what gives it away:
#     El Torito boot img :   1  UEFI  y   none ...
#
# grub-pc-bin is in PKGS, so its i386-pc tree lands in $PREFIX.  /usr/lib/grub
# is root-owned and this host has no usable root, so those modules cannot be
# placed beside the host's.  Instead a launcher in .toolchain/bin (first on
# PATH -- see scripts/env.sh) re-executes grub-mkrescue inside a private mount
# namespace whose /usr/lib/grub is a union of the host's x86_64-efi and our
# i386-pc.  The fixture Makefiles keep calling plain `grub-mkrescue`.
install_grub_mkrescue_launcher() {
  local real="/usr/bin/grub-mkrescue"
  local pc="$PREFIX/usr/lib/grub/i386-pc"
  local efi="/usr/lib/grub/x86_64-efi"
  local libdir="$TOOLCHAIN/grub-lib"
  local launcher="$TOOLCHAIN/bin/grub-mkrescue"

  if [ ! -d "$pc" ]; then
    warn "grub: $pc is missing (grub-pc-bin did not unpack)"
    warn "grub: ISOs will be UEFI-only and will NOT boot under 'qemu -boot d'"
    return 0
  fi
  [ -x "$real" ] || { warn "grub: $real is missing"; return 0; }

  mkdir -p "$libdir"
  ln -sfn "$pc" "$libdir/i386-pc"
  # The host's EFI tree is optional: without it the ISO is BIOS-only, which is
  # exactly what the P0 acceptance path needs.
  if [ -d "$efi" ]; then
    ln -sfn "$efi" "$libdir/x86_64-efi"
  else
    rm -f "$libdir/x86_64-efi"
  fi
...
# Generated by scripts/bootstrap-toolchain.sh -- do not edit by hand.
#
# Gives grub-mkrescue a private mount namespace in which /usr/lib/grub holds
# both the host's x86_64-efi and this workspace's i386-pc, so the ISO it builds
# is bootable in QEMU's BIOS mode (-cdrom ... -boot d).  Without it the host's
# EFI-only module tree yields a UEFI-only ISO that QEMU silently refuses to
# boot.  See install_grub_mkrescue_launcher() for the full story.
PREFIX="$PREFIX"
GRUBLIB="$libdir"
REAL="$real"

if [ -z "\${PRINCESSIDE_GRUB_IN_NS:-}" ]; then
  [ -d "\$GRUBLIB/i386-pc" ] || exec "\$REAL" "\$@"
  command -v unshare >/dev/null 2>&1 || exec "\$REAL" "\$@"
  PRINCESSIDE_GRUB_IN_NS=1 \\
  PRINCESSIDE_GRUB_LIB="\$GRUBLIB" \\
  PRINCESSIDE_GRUB_REAL="\$REAL" \\
  PRINCESSIDE_GRUB_PATH="\$PREFIX/usr/bin:/usr/bin:/bin" \\
    exec unshare --mount --map-root-user sh -c '
      mount --bind "\$PRINCESSIDE_GRUB_LIB" /usr/lib/grub 2>/dev/null || exit 126
      PATH="\$PRINCESSIDE_GRUB_PATH"
      export PATH
      exec "\$PRINCESSIDE_GRUB_REAL" "\$@"
    ' grub-mkrescue "\$@"
fi
exec "\$REAL" "\$@"
EOF
  chmod +x "$launcher"
  log "grub: launcher written to $launcher ($(ls "$pc" | wc -l) i386-pc modules)"
}
...
verify() {
  ...
  # `set -euo pipefail`, and these are plain assignments (not conditions), so an
  # absent clangd-14/clangd-16 aborted it here with exit 127 -- silently
  # swallowing every diagnostic below, including the one naming the real
  # problem.  A missing tool must reach the `case` statements, which is where it
  # gets reported.
  v14="$(clangd-14 --version 2>&1 | head -n1 || true)"
  v16="$(clangd-16 --version 2>&1 | head -n1 || true)"
  case "$v14" in
  ...
  if [ -x "$TOOLCHAIN/bin/princess-gdb" ]; then
    local gv gmaj
    gv="$("$TOOLCHAIN/bin/princess-gdb" --version 2>&1 | head -n1 || true)"
    case "$gv" in
      *"GNU gdb"*) log "  ok   princess-gdb --version  ->  $gv" ;;
      *) warn "  FAIL princess-gdb did not report a GNU gdb version: $gv"; fail=1 ;;
    esac
    gmaj="$(printf '%s' "$gv" | grep -oE '[0-9]+\.[0-9]+' | head -n1 | cut -d. -f1 || true)"
...
# Verification failures are reported but do not abort.  Everything installable
# is installed by this point, and some shortfalls are simply not available on
# this distribution (clangd-16 has no trixie package at all), so a hard failure
# here would only hide the "done" line and leave the user unsure whether the
# toolchain was set up.  scripts/doctor.sh is the gate that decides usability.
if ! verify; then
  warn "toolchain verification reported problems (see above)"
  warn "run scripts/doctor.sh for the full picture"
fi
